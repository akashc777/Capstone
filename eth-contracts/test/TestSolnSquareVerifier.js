...
contract('SolnSquareVerifier', accounts => {
    ...
    describe('Soln', function () {
        ...
        it('New solution should be added', async function () { 
            const instance = await SolnSquareVerifier.new({from: account_one});
            const sol = await instance.addSolution(proof.proof.a, proof.proof.b, proof.proof.c, proof.inputs);
            assert.equal(sol.logs[0].event, 'SolutionAdded', "Not able to add solution")
            solAddress = sol.logs[0].args.sol.solAddress;
        });

        // Test if an ERC721 token can be minted for contract - SolnSquareVerifier
        it('Should mint a token', async function () { 
            const instance = await SolnSquareVerifier.new({from: account_one});
          await instance.mintSq(account_one, 20, proof.proof.a, proof.proof.b, proof.proof.c, proof.inputs, {from: account_one});
          let owner = await instance.ownerOf(20);
            assert.equal( account_one, owner, "Token balance does not match");
      });
    });
});